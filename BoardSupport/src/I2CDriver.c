...
{
    // add your code
    SysCtlPeripheralEnable(SYSCTL_PERIPH_I2C0); //step 1: enable I2C periph
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOB);
    SysCtlDelay(3); //ensure I2C enabled before writing to registers
    GPIOPinConfigure(GPIO_PB2_I2C0SCL);
    GPIOPinConfigure(GPIO_PB3_I2C0SDA);
    GPIOPinTypeI2CSCL(GPIO_PORTB_BASE, GPIO_PIN_2);
    GPIOPinTypeI2C(GPIO_PORTB_BASE, GPIO_PIN_3);

    I2CMasterInitExpClk(moduleNumber, SysCtlClockGet(), false);
}

void InitializeSensorI2C()
{
    // your code
    SysCtlPeripheralEnable(SYSCTL_PERIPH_I2C1); //step 1: enable I2C periph
...
{
    // your code
    I2CMasterDataPut(moduleNumber, data); //sets byte to transmit
